...
class PriorityThreadPoolExecutor():
    ...
    def _scheduler(self):
        while not self.shutdown_flag:
            try:
                task = self.task_queue.get(timeout=1)  # 等待任务
...
class PriorityThreadPoolExecutor_BAK(ThreadPoolExecutor):
    def __init__(self, max_workers, *args, **kwargs):
...
class ExpertMemoryManager:
    def __init__(self, 
        ...
        self.overwait_count = 0         # 计数器 - 用于调整动态步长
        self.process_count = 0          # 计数器
        self.num_overwait_wind = 50     # 计数器窗口
        self.last_recent_task = {"layer_idx": 0, "exper_id": 0} # 最后处理的task
        self.high_overwait_wind_thres = self.num_overwait_wind * 0.8 # steps += 1
    ...
    def use_cache_aware(self, use_cache_aware: bool):
        ...
        # update executor
        if not use_cache_aware:
            self.task_queue = queue.Queue()
            self.executor = PriorityThreadPoolExecutor_BAK(max_workers=4)
    ...
    def _evict_experts_if_needed(self, num_experts_swap_in: Optional[int] = 0):
    ...
    def _enough_gpu_memory_space(self, num_experts_swap_in: Optional[int] = 0):
        """
        wether the gpu memory is enough for experts
        """
        if self.USE_LRU_POLICY:
            return len(self.experts_access_order) + num_experts_swap_in > self.num_experts_in_gpu * 1.0
        else:
            return len(self.experts_access_order_high) + len(self.experts_access_order_low) + \
                num_experts_swap_in > self.num_experts_in_gpu * 1.0
    ...
    def _update_device_map(self, layer_idx, expert_id, task_type):
        ...
        if task_type == "swap_in":
            ...
            self.cur_experts_device_map[layer_idx][expert_id] = "cuda"
            self.swap_in_tasks[layer_idx][expert_id] = None
            self.last_recent_task = {"layer_idx": layer_idx, "exper_id": expert_id}
    ...
    def _iter(self, layer_idx, expert_id_list):
        ...
        return expert_id_list[0]

    def update_count(self, wait_lat, layer_idx):
        """handle the counter"""
        self.process_count += 1
        if wait_lat > 0.0:
            self.overwait_count += 1
            if self.overwait_count > self.high_overwait_wind_thres:
                self.update_skip_steps(self.num_skip_steps + 1)
                self.num_skip_steps += 1
        else:
            pass
            # if self.last_recent_task["layer_idx"] > self.num_skip_steps and \
            #     self.last_recent_task["layer_idx"] > layer_idx + 2:
            #     self.update_skip_steps(self.num_skip_steps - 1)
            #     self.num_skip_steps -= 1

        # reset count
        if self.process_count >= self.num_overwait_wind:
            self.process_count = 0
            self.overwait_count = 0

    def wait_for_expert(self, layer_idx, expert_ids):
        """
        ...
        # get expert_id from iter
        item = self._iter(layer_idx, expert_ids)
        if isinstance(item, int):
            expert_id = item
        else:
            expert_id, _, _ = item
        ...
        # dynamic steps
        self.update_count(wait_lat, layer_idx)

        self.experts_module_list[layer_idx][expert_id].to(self.gpu_device)

        return item, wait_lat, cache_mis_lat

    def wait_for_expert_bak(self, layer_idx, expert_id):
        """
        waiting for the asy- expert load finish
        if no task and not in GPU, means cache mis, create an swap_in task
        """
    ...
    def prefetch(self, 
        ...
        if self.USE_LRU_POLICY and self._enough_gpu_memory_space(len(expert_id_list)):
        ...
        for expert_id in expert_id_list:
            ...
            # already in tasks?
            if not self.USE_LRU_POLICY and self.swap_in_tasks[layer_idx][expert_id] is not None:
                continue

            if self._enough_gpu_memory_space(1):
                self._evict_experts_if_needed(1)
